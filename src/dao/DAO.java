...
public class DAO {
   ...
   public static Authentification connexion(Authentification user) throws SQLException
   {
	...
	ResultSet rs=SqlData.selectQuery("select * from authentification where lower(login)=lower('"+user.getLogin()+"') and lower(password)=lower('"+user.getPassword()+"');");
	try {
		if(rs.next()==false)
   ...
   public static Authentification connexion(String login, String password) 
   {
	   SqlData.connection();
	   Authentification u=null;
		ResultSet rs=SqlData.selectQuery("select * from authentification where lower(login)=lower('"+login+"') and lower(password)=lower('"+password+"');");
   ...
   public static Utilisateur getUserByRFID(String id)
   {
	   ...
	   try {
		if(rs.next())
		   {
			   u=new Utilisateur(rs.getInt("idU"),rs.getString("nom"),rs.getString("prenom"),rs.getString("ddn"),rs.getLong("ndt"),rs.getString("email"),rs.getString("id"),rs.getInt("idR"),rs.getString("matricule"),rs.getBoolean("admin"));   
			   SqlData.disconection();
		   }
		else
			SqlData.disconection();
   ...
   {
	   SqlData.connection();
	   SqlData.miseAJour("insert into utilisateurs (nom,prenom,ddn,ndt,email,id,idR,matricule) values ('"+u.getNom()+"','"+u.getPrenom()+"',STR_TO_DATE('"+u.getDdn()+"','%d-%m-%Y'),"+u.getNdt()+",'"+u.getEmail()+"','"+u.getId()+"',"+u.getidR()+",'"+u.getMatricule()+"')");
       SqlData.disconection();
   }
   public static void updateAuthentification(String id,String login, String password)
   {
	   SqlData.connection();
	   SqlData.miseAJour("update authentification set login='"+login+"',password='"+password+"' where id='"+id+"';");
	   SqlData.disconection();   
   }
   public static void updateAuthentification(Authentification a)
   {
	   SqlData.connection();
	   SqlData.miseAJour("update authentification set login='"+a.getLogin()+"',password='"+a.getPassword()+"' where id='"+a.getId()+"';");
	   SqlData.disconection();   
   }
   public static void creeVehicule(Vehicule v)  {
	   SqlData.connection();
	   SqlData.miseAJour("insert into vehicules (matricule,marque,ndp) values ('"+v.getMatricule()+"','"+v.getMarque()+"',"+v.getNdp()+")");
	   SqlData.disconection();   
   }
   public static void creeVehicule(String matricule,String marque,int ndp)  {
	   SqlData.connection();
	   SqlData.miseAJour("insert into vehicules (matricule,marque,ndp) values ('"+matricule+"','"+marque+"',"+ndp+")");
	   SqlData.disconection();   
   }
   ...
   {
	   SqlData.connection();
	   ResultSet rs=SqlData.selectQuery("select * from cartiers where lower(nomC)=lower('"+c.getNomC()+"') and idR="+c.getIdR()+";");
	   int i=-1;
	   try {
		if(!rs.next())
		{  
		   ...
		   rs=SqlData.selectQuery("select * from cartiers where lower(nomC)=lower('"+c.getNomC()+"') and idR="+c.getIdR()+";");
		   rs.next();
		   i=rs.getInt("idC");
		}else {
			i=rs.getInt("idC");
   ...
   {
	SqlData.connection();
	SqlData.miseAJour("insert into trajets (bagageAutorise,prix,dateDepart,idCreateur,idCartierDepart,idCartierArrive) values ("+t.isBagageAutorise()+","+t.getPrix()+",STR_TO_DATE('"+t.getDateDepart()+"','%d-%m-%Y'),"+t.getIdCreateur()+","+t.getIdCartierDepart()+","+t.getIdCartierArrive()+");");
	SqlData.disconection();
   }
   public static List<Trajet> rechercherTrajets(int idDepart,int idArrive)
   {
	   SqlData.connection();
	   ResultSet rs=SqlData.selectQuery("select * from trajets where idCartierDepart in (select idC from cartiers where idR="+idDepart+") and idCartierArrive in (select idC from cartiers where idR="+idArrive+") order by dateDepart;");
	  List<Trajet> t=new ArrayList<Trajet>();
	   try {
		while(rs.next())
			    t.add(new Trajet(rs.getInt(1),rs.getString(2), rs.getBoolean(3), rs.getInt(4), rs.getString(5), rs.getInt(6),rs.getInt(7), rs.getInt(8)));
	} catch (SQLException e) {
   ...
   {
	   SqlData.connection();
	   ResultSet rs=SqlData.selectQuery("select nomR from regions where idR in (select idR from cartiers where idC="+idC+")");
	   String s;
	   try {
		rs.next();
		s=rs.getString("nomR");
   ...
   public static Trajet getTrajetById(int id)
   {
	   ...
	   try {
		rs.next();
		t=new Trajet(rs.getInt(1),rs.getString(2), rs.getBoolean(3), rs.getInt(4), rs.getString(5), rs.getInt(6),rs.getInt(7), rs.getInt(8));
	    } catch (SQLException e) {
		e.printStackTrace();
      	}
   ...
   public static Vehicule getVehiculeByIdU(int idU)
   {
	   ...
	   SqlData.connection();
	   ResultSet rs=SqlData.selectQuery("select * from vehicules where matricule in (select matricule from utilisateurs where idU="+idU+");");
	   Vehicule t = null;
	   try {
		rs.next();
   ...
   public static List<Reservation> getReservationByTrajetCreator(int idU){
	   List<Reservation> l=new ArrayList<>();
	   SqlData.connection();
	   ResultSet rs=SqlData.selectQuery("select * from reservations where idT in (select idT from trajets where idCreateur="+idU+") order by idT;");
	   try {
		while(rs.next())
		   {
			   l.add(new Reservation(rs.getInt(1),rs.getString(2),rs.getInt(3),rs.getInt(4),rs.getInt(5)));
